...
@cocotb.coroutine
async def monitor_outputs(dut, exp):
    """Monitor outputs."""
    await RisingEdge(dut.ready)
    actual = to_signed(int(dut.Product), 32)
    if actual != exp:
        dut._log.error(f"Test failed: A={to_signed(int(dut.Multiplicand), 16)}, B={to_signed(int(dut.Multiplier), 16)}, Product={actual}, Expected={exp}")
    else:
...
async def test_seq_mul(dut):
    ...
    for i in range(400):
        ...
        monitor_task = cocotb.start_soon(monitor_outputs(dut, exp))
        await drive_task
        await monitor_task
