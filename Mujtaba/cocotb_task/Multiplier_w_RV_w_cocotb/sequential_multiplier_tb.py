...
@cocotb.test()
async def sequential_multiplier_tb(dut):
    # Clock generation
    ...
    dut._log.info(f"Multiplicand: {dut.Multiplicand.value.signed_integer} Multiplier: {dut.Multiplier.value.signed_integer} Product: {dut.Product.value.signed_integer}")
    await directed_test(dut, 0, 3)
    dut._log.info(f"Multiplicand: {dut.Multiplicand.value.signed_integer} Multiplier: {dut.Multiplier.value.signed_integer} Product: {dut.Product.value.signed_integer}")
    await directed_test(dut, -2, 3)
    dut._log.info(f"Multiplicand: {dut.Multiplicand.value.signed_integer} Multiplier: {dut.Multiplier.value.signed_integer} Product: {dut.Product.value.signed_integer}")
...
async def monitor(dut):
    ...
    for _ in range(200000):
        ...
        expected_product = dut.Multiplicand.value.signed_integer * dut.Multiplier.value.signed_integer
        
        await RisingEdge(dut.dest_ready)

        # Check the product against the expected value
        if dut.Product.value.signed_integer != expected_product:
            dut._log.error(f"Error: Expected product {expected_product}, but got {int(dut.Product.value.signed_integer)}")
        else:
            count_pass += 1
